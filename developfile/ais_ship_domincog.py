...
line_ship, = ax.plot([], [], color='k')
time_text = ax.text(-1500, 1550, '', fontsize=10)
speed_text = ax.text(-1500, 1400, '', fontsize=10)
cog_text = ax.text(-1500, 1250, '', fontsize=10)
...
def animate(i):
    ...
    line_ship.set_data(ship_ver[:, 0], ship_ver[:, 1])
    ...
    cog_text.set_text('cog: {0} deg'.format(str(cog_ship[i])))

    return line1, line2, line3, line4


anim = FuncAnimation(fig, animate, init_func=init,
                               frames=int(len(cog_ship)/3), interval=100 , blit=True)
if alpha0 == True:
